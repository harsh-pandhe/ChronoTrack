...
async function enforce(key, limit, windowMs) {
  // Test bypass: the integration suite legitimately drives >10 logins from one
  // loopback IP inside a minute, which would otherwise trip the login limiter on
  // a fast runner (and only there — a slow/remote DB lets the 60s window reset,
  // which is exactly the kind of environment-dependent flake to eliminate).
  // Only ever set in CI's test job / local test runs, never in prod.
