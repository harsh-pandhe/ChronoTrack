...
// Confirm a code matches the enrolled secret WITHOUT advancing last_step. Used
// during enrollment (totp-activate): consuming the step there would reject the
// user's very next login if it happened within the same 30s window, since it'd
// be the same code. Replay protection only matters for authentication, and the
// first real login sets the last_step baseline.
export async function checkTotp(userId, token) {
  const { rows } = await query(`SELECT secret_enc FROM mfa_totp WHERE user_id=$1`, [userId]);
  if (!rows[0]) return false;
  return totpValidate(decryptSecret(rows[0].secret_enc), token) !== null;
}

// Verify a TOTP code for LOGIN. Atomic: reads last_step under a row lock, checks
...
// Normalize the SAME way codes are hashed at creation (strip hyphens AND
// whitespace, uppercase) so a user can paste the pretty XXXX-XXXX-... form.
