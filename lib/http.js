...
export async function requireAuth(req, roles = null) {
  ...
  // A "MFA pending" token is issued after a correct password but BEFORE the
  // second factor — it carries a real `sub`, so without this guard it would
  // sail through as a full session. It may ONLY be presented to the MFA verify
  // endpoint, which validates it directly. Reject it on *presence* of the flag,
  // never on absence of a session marker: existing 8h user tokens and device
  // tokens have no `typ` and must keep working.
  if (claims.typ === 'mfa_pending') throw new HttpError(401, 'MFA not completed');
