            ...
            {activeTlTab === 'manage' && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <form onSubmit={async (e) => {
                      ...
                      try {
                        ...
                        setPendingActivations((prev) => [
                          { user_id: created.id, name: newEmpName, email: newEmpEmail, code, status: 'Pending' },
                          ...prev,
                        ]);
                        await loadServerData();
                        showToast(`${newEmpName} registered — activation code shown below.`, 'success');
                        setNewEmpName('');
                        setNewEmpEmail('');
                      } catch (err) {
                  ...
                  {/* Codes issued this session — persistent so the lead can copy
                      the 8-digit code (it's only ever shown once; stored hashed at
                      rest) instead of racing a toast that vanishes in seconds. */}
                  {pendingActivations.length > 0 && (
                    <div className="md:col-span-2 p-6 rounded-xl bg-emerald-500/5 border border-emerald-500/20 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-semibold uppercase text-emerald-500 tracking-wider">Activation codes issued — copy now, shown once</span>
                        <button onClick={() => setPendingActivations([])} className="text-[10px] text-muted-foreground hover:text-foreground uppercase tracking-wider">Clear</button>
                      </div>
                      <div className="space-y-2">
                        {pendingActivations.map((p, i) => (
                          <div key={p.user_id || i} className="flex items-center justify-between gap-3 rounded-xl bg-card border border-border px-4 py-2.5">
                            <div className="min-w-0">
                              <span className="block text-xs font-semibold text-foreground truncate">{p.name}</span>
                              <span className="block text-[10px] text-muted-foreground truncate">{p.email}</span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <code className="text-base font-bold font-mono tracking-[0.2em] text-primary tabular-nums">{p.code}</code>
                              <button
                                onClick={() => { navigator.clipboard?.writeText(p.code); showToast('Code copied.', 'success'); }}
                                title="Copy code"
                                className="p-1.5 rounded-lg border border-border bg-muted hover:text-foreground text-muted-foreground transition-colors"
                              >
